...
def compute_confluence_score(
    ...
    if sentiment_regime in ("positive", "strongly_positive"):
        score += 15
    elif sentiment_regime == "neutral":
        score += 7
...
def run_technicals(output_dir: str = "api-output") -> None:
    ...
    if "error" not in data_1h and data_1h["candles"]:
        ...
        # Sentiment regime placeholder (integrate with sentiment_daily.json if available)
        sentiment_regime = _read_sentiment_regime(output_path)

        score = compute_confluence_score(
            states, sentiment_regime, support_zones, current_price, rsi_current
        )
...
def _read_sentiment_regime(output_path: Path) -> str:
    """Read latest sentiment from existing sentiment_daily.json if present."""
    try:
...
def main():
    ...
    print("📊 BTC Technicals Pipeline")
    print("=" * 60)
    print(f"Output: {output_dir}")
    print()

    try:
        run_technicals(output_dir)
        print()
        print("=" * 60)
        print("✅ Technicals pipeline complete!")
    except Exception as e:
